...
int query(int node, int l, int r, int lx, int x) {
	if(tree[node] < x || r < lx) return -1;
	if(l == r) return l;
	int m = (l+r)/2;
	int ans = query(2*node,l,m,lx,x);
	if(ans == -1) return query(2*node+1,m+1,r,lx,x);
	return ans;
}
